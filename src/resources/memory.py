"""In-memory resource catalog store."""
...
class MemoryResourceStore(ResourceStore):
    ...
    async def update_resource(
        ...
        if resolved.content is not None:
            next_version_number = next_record.current_version.version + 1
    ...
    def _resolve_create(self, resource: ResourceCreate | dict[str, Any]) -> ResourceCreate:
        return resource if isinstance(resource, ResourceCreate) else ResourceCreate.model_validate(resource)

    def _make_record(self, resource: ResourceCreate) -> ResourceRecord:
