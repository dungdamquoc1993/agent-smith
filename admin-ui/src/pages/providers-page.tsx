...
export function ProvidersPage() {
  ...
  return <>
    ...
    <div className="mt-8">
      {providers.isPending ? <LoadingState label="Loading identity providers…" /> : providers.isError ? <ErrorNotice error={providers.error} /> : rows.length === 0 ? <Card><CardContent className="flex flex-col items-center py-14 text-center"><span className="rounded-full bg-blue-50 p-4 text-blue-700"><ServerCog className="h-7 w-7" /></span><h2 className="mt-4 text-lg font-semibold">No identity providers</h2><p className="mt-2 max-w-md text-sm text-muted-foreground">Create your first trusted provider to issue API keys or signed assertions.</p><Button className="mt-5" onClick={() => setCreateOpen(true)}><Plus className="h-4 w-4" />Create provider</Button></CardContent></Card> : <>
        <div className="overflow-hidden rounded-lg border bg-white shadow-card"><div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="border-b bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-5 py-3 font-medium">Provider</th><th className="px-5 py-3 font-medium">Issuer</th><th className="px-5 py-3 font-medium">Status</th><th className="px-5 py-3 font-medium">Updated</th><th className="px-5 py-3"><span className="sr-only">Open</span></th></tr></thead><tbody className="divide-y">{rows.map((provider) => <tr key={provider.id} className="hover:bg-slate-50"><td className="px-5 py-4"><p className="font-medium text-slate-950">{provider.displayName}</p><p className="mt-0.5 font-mono text-xs text-muted-foreground">{provider.slug}</p></td><td className="max-w-xs truncate px-5 py-4 text-slate-600">{provider.issuer}</td><td className="px-5 py-4"><Badge>{provider.status}</Badge></td><td className="whitespace-nowrap px-5 py-4 text-slate-600">{formatDate(provider.updatedAt)}</td><td className="px-5 py-4 text-right"><Link className="inline-flex items-center gap-1 font-medium text-blue-700 hover:underline" to={`/providers/${provider.id}`}>Manage<ArrowRight className="h-4 w-4" /></Link></td></tr>)}</tbody></table></div></div>
...
function CreateProviderDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (value: boolean) => void }) {
  ...
  return <Dialog open={open} onOpenChange={changeOpen}><DialogContent className="max-h-[90vh] overflow-y-auto"><DialogTitle>Create identity provider</DialogTitle><DialogDescription>Add a trusted issuer. You can create credentials after the provider is saved.</DialogDescription>
    <form className="mt-5 space-y-4" onSubmit={form.handleSubmit((values) => mutation.mutate(values))} noValidate>
      ...
      <Field label="Status" id="create-status"><select id="create-status" className="h-10 w-full rounded-md border bg-white px-3 text-sm" {...form.register("status")}><option value="active">Active</option><option value="disabled">Disabled</option></select></Field>
