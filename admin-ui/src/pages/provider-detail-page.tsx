...
export function ProviderDetailPage() {
  ...
  if (query.isError) {
    if (query.error instanceof ApiError && query.error.status === 404) return <div><Link className="inline-flex items-center gap-2 text-sm font-medium text-blue-700" to="/providers"><ArrowLeft className="h-4 w-4" />Identity providers</Link><div className="mt-8"><ErrorNotice error={query.error} title="Provider not found" /></div></div>
    return <ErrorNotice error={query.error} />
  }
  const provider = query.data.identityProvider
  return <>
    <Link className="inline-flex items-center gap-2 text-sm font-medium text-slate-600 hover:text-slate-950" to="/providers"><ArrowLeft className="h-4 w-4" />Identity providers</Link>
    ...
    <div className="mt-8 space-y-6">
      <Card><CardHeader><CardTitle>Provider overview</CardTitle><CardDescription>Changes take effect for new authentication attempts immediately.</CardDescription></CardHeader><CardContent><form className="grid gap-5 sm:grid-cols-2" onSubmit={form.handleSubmit((values) => update.mutate(values))} noValidate>
        ...
        {update.isSuccess && !form.formState.isDirty && <div role="status" className="rounded-md border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800 sm:col-span-2">Provider saved.</div>}
        ...
        <Field label="Status" id="detail-status"><select id="detail-status" className="h-10 w-full rounded-md border bg-white px-3 text-sm" {...form.register("status")}><option value="active">Active</option><option value="disabled">Disabled</option></select></Field>
