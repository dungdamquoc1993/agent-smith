...
export function AuditPage() {
  ...
  return <>
    ...
    <Card className="mt-8"><CardContent className="pt-5 sm:pt-6"><form className="grid gap-4 md:grid-cols-2 xl:grid-cols-5" onSubmit={(event) => { event.preventDefault(); setFilters(draft) }}>
      ...
      <AuditField label="Outcome" id="audit-outcome"><select id="audit-outcome" className="h-10 w-full rounded-md border bg-white px-3 text-sm" value={draft.outcome} onChange={(event) => update("outcome", event.target.value)}><option value="">All outcomes</option><option value="success">Success</option><option value="denied">Denied</option><option value="failed">Failed</option></select></AuditField>
    ...
    <div className="mt-6">{query.isPending ? <LoadingState label="Loading audit events…" /> : query.isError ? <ErrorNotice error={query.error} /> : !rows.length ? <Card><CardContent className="flex flex-col items-center py-14 text-center"><ScrollText className="h-8 w-8 text-slate-400" /><h2 className="mt-4 text-lg font-semibold">No audit events found</h2><p className="mt-2 text-sm text-muted-foreground">Try changing the filters or check again after admin activity.</p></CardContent></Card> : <>
      <div className="overflow-hidden rounded-lg border bg-white shadow-card"><div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="border-b bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-4 py-3 font-medium">Time</th><th className="px-4 py-3 font-medium">Action</th><th className="px-4 py-3 font-medium">Outcome</th><th className="px-4 py-3 font-medium">Actor</th><th className="px-4 py-3 font-medium">Resource</th><th className="px-4 py-3 font-medium">Details</th></tr></thead><tbody className="divide-y">{rows.map((event) => <tr key={event.id} className="align-top"><td className="whitespace-nowrap px-4 py-4 text-xs text-slate-600">{formatDate(event.occurredAt)}</td><td className="px-4 py-4 font-mono text-xs">{event.action}</td><td className="px-4 py-4"><Badge>{event.outcome}</Badge></td><td className="px-4 py-4"><p className="text-sm">{event.actor.identifier || event.actor.kind}</p>{event.actor.operatorId && <p className="mt-1 font-mono text-xs text-muted-foreground">{event.actor.operatorId}</p>}</td><td className="px-4 py-4"><p>{event.resourceType}</p>{event.resourceId && <p className="mt-1 max-w-52 truncate font-mono text-xs text-muted-foreground" title={event.resourceId}>{event.resourceId}</p>}</td><td className="px-4 py-4"><details><summary className="cursor-pointer text-sm font-medium text-blue-700">View metadata</summary><pre className="mt-3 max-w-sm overflow-auto rounded-md bg-slate-950 p-3 text-xs text-slate-50">{JSON.stringify({ ...event.metadata, requestId: event.actor.requestId, ipAddress: event.actor.ipAddress }, null, 2)}</pre></details></td></tr>)}</tbody></table></div></div>
