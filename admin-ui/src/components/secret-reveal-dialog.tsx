...
function OpenSecretRevealDialog({ label, secret, onClose }: { label: string; secret: string; onClose: () => void }) {
  ...
  return <Dialog open onOpenChange={(next) => { if (!next && saved) onClose() }}>
    <DialogContent hideClose onEscapeKeyDown={(event) => { if (!saved) event.preventDefault() }} onPointerDownOutside={(event) => event.preventDefault()}>
      <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-full bg-amber-100 text-amber-700 dark:bg-amber-500/15 dark:text-amber-300"><KeyRound className="h-5 w-5" /></div>
      ...
      <label className="mt-5 flex cursor-pointer items-start gap-3 rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-950 dark:border-amber-800/70 dark:bg-amber-950/40 dark:text-amber-100">
        <input className="mt-0.5 h-4 w-4" type="checkbox" checked={saved} onChange={(event) => setSaved(event.target.checked)} />
        <span><span className="flex items-center gap-1.5 font-medium"><TriangleAlert className="h-4 w-4" />I saved this credential</span><span className="mt-1 block text-xs leading-5 text-amber-800 dark:text-amber-300">I understand it cannot be viewed again.</span></span>
      </label>
