...
export function ApiKeysSection({ providerId }: { providerId: string }) {
  ...
  return <Card><CardHeader className="flex-row items-start justify-between space-y-0"><div><CardTitle className="flex items-center gap-2"><Key className="h-5 w-5 text-blue-700 dark:text-blue-300" />API keys</CardTitle><CardDescription className="mt-2">Authenticate direct API access for this provider.</CardDescription></div><Button size="sm" onClick={() => setCreateOpen(true)}><Plus className="h-4 w-4" />Create</Button></CardHeader><CardContent>
...
export function AssertionKeysSection({ providerId }: { providerId: string }) {
  ...
  return <Card><CardHeader className="flex-row items-start justify-between space-y-0"><div><CardTitle className="flex items-center gap-2"><ShieldEllipsis className="h-5 w-5 text-violet-700 dark:text-violet-300" />Assertion keys</CardTitle><CardDescription className="mt-2">Sign trusted parent-application identity assertions.</CardDescription></div><Button size="sm" onClick={() => setCreateOpen(true)}><Plus className="h-4 w-4" />Create</Button></CardHeader><CardContent>
