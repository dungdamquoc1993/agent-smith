...
export function AppShell({ operator, children }: { operator: Operator; children: React.ReactNode }) {
  ...
  return <div className="min-h-screen lg:grid lg:grid-cols-[260px_1fr]">
    <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b bg-white px-4 lg:hidden">
      <Brand compact /><Button variant="ghost" size="icon" onClick={() => setMobileOpen((value) => !value)} aria-label="Toggle navigation">{mobileOpen ? <X /> : <Menu />}</Button>
    </header>
    <aside className={cn("fixed inset-x-0 top-16 z-20 border-b bg-white p-4 shadow-lg lg:sticky lg:top-0 lg:block lg:h-screen lg:border-b-0 lg:border-r lg:p-5 lg:shadow-none", mobileOpen ? "block" : "hidden")}>
...
function Brand({ compact = false }: { compact?: boolean }) {
  return <div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-lg bg-slate-950 text-white"><ShieldCheck className="h-5 w-5" /></span><div><p className="text-sm font-semibold leading-none">Agent Smith</p>{!compact && <p className="mt-1 text-xs text-muted-foreground">Control plane</p>}</div></div>
}
